// Smoke test: connect to OpenAI Realtime transcription, push 1 second of
// silence, wait for session.updated, then close. Confirms auth + protocol
...
const main = async () => {
    ...
    const done = new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
            ws.close();
            reject(new Error('Timed out waiting for transcription_session.updated'));
        }, 10_000);
