#!/usr/bin/env bash
#
# Waiting for Godot — production deploy script.
#
# Assumes you have:
#   - gcloud auth configured against project cts-development-485012
#   - firebase CLI authenticated for the same project
#   - All four production secrets ready (see SECRETS_FILE)
#
...
SERVICE_NAME="${SERVICE_NAME:-wfg-server}"
...
upload_secret WFG_ANTHROPIC_API_KEY "${ANTHROPIC_API_KEY}"
upload_secret WFG_OPENAI_API_KEY "${OPENAI_API_KEY}"
upload_secret WFG_GEMINI_API_KEY "${GEMINI_API_KEY}"
upload_secret WFG_ADMIN_PASSWORD "${ADMIN_PASSWORD}"
upload_secret WFG_SESSION_COOKIE_SECRET "${SESSION_COOKIE_SECRET}"
...
gcloud --project="${PROJECT_ID}" run deploy "${SERVICE_NAME}" \
    ...
    --port=8080 \
    --timeout=3600 \
    --set-env-vars="NODE_ENV=production,GCLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_PROJECT=${PROJECT_ID},SESSION_ID=change-makers-2026,PUBLIC_BASE_URL=https://creativethinkingsystems.com" \
    --set-secrets="ANTHROPIC_API_KEY=WFG_ANTHROPIC_API_KEY:latest,OPENAI_API_KEY=WFG_OPENAI_API_KEY:latest,GEMINI_API_KEY=WFG_GEMINI_API_KEY:latest,ADMIN_PASSWORD=WFG_ADMIN_PASSWORD:latest,SESSION_COOKIE_SECRET=WFG_SESSION_COOKIE_SECRET:latest"
...
curl -fsS "${SERVICE_URL}/wfg-api/health" || { echo "[fail] health check failed" >&2; exit 1; }
echo ""

echo ""
echo "[deploy] next step — preview the Hosting changes:"
echo "         firebase hosting:channel:deploy wfg-preview --project=${PROJECT_ID} --expires=2d"
echo "         (this requires firebase.json to already include the wfg rewrites — see DEPLOY.md)"
echo ""
echo "[deploy] done."
