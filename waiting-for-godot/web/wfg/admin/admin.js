// WFG admin console. Single-page vanilla JS.
...
(() => {
    ...
    const godotCapDisplay = $('#godot-cap-display');
    ...
    const pushTranscriptSegment = ({ text, at, source }) => {
        // Clear "empty" placeholder if first segment.
    ...
    const api = async (path, opts = {}) => {
        const res = await fetch(`/wfg-api${path}`, {
    ...
    const renderProfile = (p) => {
        ...
        card.innerHTML = `
            <h3>${av}<span>${escape(p.displayName)}${p.nickname ? ` <span style="color:#888;font-weight:400">“${escape(p.nickname)}”</span>` : ''}</span> <span class="pill">${escape(p.modelLabel || p.model)}</span></h3>
            <div class="grid">
    ...
    const renderSettings = (settings) => {
        // Populate fields across any form on the page that names them.
        for (const [k, v] of Object.entries(settings)) {
            for (const el of document.querySelectorAll(`form input[name="${k}"]`)) {
                el.value = el.dataset.unit === 'seconds' ? v / 1000 : v;
            }
        }
        godotCapDisplay.textContent = settings.godotDeliberationCap;
    ...
    const renderAdminFeed = (posts) => {
        ...
        // Show last 20, newest at top (we use flex-direction column-reverse).
    ...
    const adminPostEl = (post) => {
        ...
        wrap.innerHTML = `
            ...
            <div>
              ...
              <div class="text">${escape(post.body)}</div>
            </div>
        `;
        return wrap;
    };
    ...
    const startMic = async () => {
        ...
        await micCtx.audioWorklet.addModule('/wfg/admin/pcm-worklet.js');
        const source = micCtx.createMediaStreamSource(micStream);
        const node = new AudioWorkletNode(micCtx, 'pcm-worklet');

        const wsProto = location.protocol === 'https:' ? 'wss:' : 'ws:';
        micWs = new WebSocket(`${wsProto}//${location.host}/wfg-api/ws/audio`);
    ...
    $('#godot-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const q = $('#godot-question').value.trim();
        if (!q) return;
        try {
            await api('/admin/godot', { method: 'POST', body: JSON.stringify({ question: q }) });
            $('#godot-question').value = '';
        } catch (err) { alert(err.message); }
    });
...
${escape(r.userMessage || '')}</pre>
            ...
            probeResult.innerHTML = `<div class="err">${escape(err.message)}</div>`;
        } finally {
            btn.disabled = false;
        }
    });
    ...
    $('#mic-stop-btn').addEventListener('click', () => stopMic());
    $('#audio-stop-btn').addEventListener('click', async () => {
    ...
    audienceToggle.addEventListener('change', async () => {
        try {
            await api('/admin/settings', {
                method: 'POST',
                body: JSON.stringify({ audienceGodotEnabled: audienceToggle.checked }),
            });
            await refreshAudienceQueue();
        } catch (err) { alert(err.message); }
    });
    ...
    $('#audience-clear').addEventListener('click', async () => {
        if (!confirm('Clear all queued audience questions?')) return;
        try {
    ...
    $('#play-file-btn').addEventListener('click', async () => {
